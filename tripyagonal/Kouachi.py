...
class Kouachi:
	...
	def __init__(self, mat):
		...
		if self.__d_size == 1:
			...
			self.__d = list(map(lambda x: sqrt(x),__dict))
		else:
			print(__dict)
	...
	def __case1(self):
		...
		for k in range(1,self.__n+1):
			if k <= self.__m:
				#num1 = (self.__b_1+self.__b_2)
				#num2 = -sqrt((self.__b_1-self.__b_2)**2)
				#num22 = 16*self.__d_sq
				#num23 = (cos(self.__theta_ks[k-1])**2)
				#print(num22,"|",num23)
				lambda_k = ((self.__b_1+self.__b_2)-sqrt((self.__b_1-self.__b_2)**2+16*self.__d_sq*(cos(self.__theta_ks[k-1])**2)))/2
			elif k <= 2*self.__m:
				lambda_k = ((self.__b_1+self.__b_2)+sqrt((self.__b_1-self.__b_2)**2+16*self.__d_sq*(cos(self.__theta_ks[k-1])**2)))/2
			else:
				lambda_k = self.__b_1
	...
	def __case2(self):
		...
		for k in range(1,self.__n+1):
			if k <= self.__m-1:
				...
				lambda_k = ((self.__b_1 + self.__b_2 - self.__alpha - self.__beta) - sqrt((self.__b_1-self.__b_2)**2 + (self.__alpha+self.__beta)**2 - 2*(self.__b_1-self.__b_2)*(self.__alpha-self.__beta)))/2
			else:
				lambda_k = ((self.__b_1 + self.__b_2 - self.__alpha - self.__beta) + sqrt((self.__b_1-self.__b_2)**2 + (self.__alpha+self.__beta)**2 - 2*(self.__b_1-self.__b_2)*(self.__alpha-self.__beta)))/2
	...
	def __case4(self):
		...
		if self.__n % 2 == 1:
			...
			if self.__alpha == self.__beta == 0:
				...
				for k in range(1,self.__n+1):
					if k <= self.__m:
						lambda_k = self.__b_1 + sqrt(self.__d_sq[0] + self.__d_sq[1] +2*self.__d[0]*self.__d[1]*cos(self.__theta_ks[k-1]))
					elif k <= 2*self.__m:
						lambda_k = self.__b_1 - sqrt(self.__d_sq[0] + self.__d_sq[1] +2*self.__d[0]*self.__d[1]*cos(self.__theta_ks[k-1]))
					else:
						lambda_k = self.__b_1
					self.__lambda_ks.append(factor(lambda_k))
			# Thm 3.2
			elif self.__alpha == self.__d[1] and self.__beta == self.__d[0] or self.__alpha == -self.__d[1] and self.__beta == -self.__d[0]:
				for k in range(1,self.__n+1):
					if k <= self.__m:
			...
			# Thm 3.3 
				...
				raise ValueError('TridiagonalMatrix cannot be solved using Kouachi\'s methods')
				raise Exception()
		# if n is even
		else:
			...
			if self.__alpha*self.__beta == self.__d_sq[1]:
				...
				for k in range(1,self.__n+1):
					if k <= self.__m-1:
						...
						lambda_k = self.__b_1 + (-(self.__alpha+self.__beta) + sqrt((self.__alpha-self.__beta)**2 + 4*self.__d_sq[0]))/2						
					else:
						lambda_k = self.__b_1 + (-(self.__alpha+self.__beta) - sqrt((self.__alpha-self.__beta)**2 + 4*self.__d_sq[0]))/2
					self.__lambda_ks.append(factor(expand(lambda_k)))
	...
	def eigenvalues(self):	
		if not self.__lambda_ks:
			if self.__d_size == 2 and self.__b_1 == self.__b_2:
				...
				print('Case I')
				self.__case1()
			elif self.__d_size == 1 and self.__n % 2 == 0 and self.__d_size == 1 and (self.__alpha*self.__beta).equals(self.__d_sq[0]):
				print('Case II')
				self.__case2()
			else:
				raise ValueError('TridiagonalMatrix cannot be solved using Kouachi\'s methods')
	...
	def eigenvectors(self):
		...
		for r in range(1,self.__n+1):
			...
			for c in range(1,self.__n+1):
				zeta_r_c = []
				zeta_r_c.append(self.__b_1 - self.__lambda_ks[c-1])
				zeta_r_c.append(self.__b_2 - self.__lambda_ks[c-1])
				if r == 1:
					if c > self.__n-2:
						self.__ev[r-1,c-1] = 1
					else:
					...
					if self.__n % 2 == 1: # if odd matr
						self.__ev[r-1,c-1] = (self.__d_sq*sin((self.__n-r+2)*self.__theta_ks[c-1]))-((self.__beta*zeta_r_c[1]-self.__d_sq)*sin((self.__n-r)*self.__theta_ks[c-1]))
						self.__ev[r-1,c-1] *= rho_r
					else: # even matr
						self.__ev[r-1,c-1] = ((zeta_r_c[1]-self.__beta)*sin((self.__n-r+1)*self.__theta_ks[c-1]))-(self.__beta*sin((self.__n-r-1)*self.__theta_ks[c-1]))
						self.__ev[r-1,c-1] *= rho_r
					if c > self.__n-2:
						if r == 2:
							self.__ev[r-1,c-1] = (-zeta_r_c[0]+self.__alpha)/self.__super[r-2]
						else:
							self.__ev[r-1,c-1] = ((-zeta_r_c[r%2]*self.__ev[r-2,c-1])-(self.__sub[r-3]*self.__ev[r-3,c-1]))/self.__super[r-2]
				else: #row is even
					if self.__n % 2 == 1: #if odd matr
						self.__ev[r-1,c-1] = ((zeta_r_c[0]-beta)*sqrt(d_sq)*sin((n-r+1)*theta_ks[c-1]))-(beta*sqrt(d_sq)*sin((n-r-1)*theta_ks[c-1]))
						self.__ev[r-1,c-1] *= rho_r
					else: # if even matr
						self.__ev[r-1,c-1] = sqrt(self.__d_sq)*sin((self.__n-r+2)*self.__theta_ks[c-1]) - (self.__beta*zeta_r_c[0]/sqrt(self.__d_sq) - sqrt(self.__d_sq)) * sin((self.__n-r)*self.__theta_ks[c-1])
						self.__ev[r-1,c-1] *= rho_r
					if c > self.__n-2:
						if r == 2:
							self.__ev[r-1,c-1] = (-zeta_r_c[0]+self.__alpha)/self.__super[r-2]
